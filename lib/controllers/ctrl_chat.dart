...
class CtrlChat extends GetxController {
  ...
  Future<GlobalResponse> actionListChatUser(BuildContext ctx, int page, String userId) async {
    try {
      ...
      SqlChat chat = chatBox.values
          .firstWhere((e) => (e.toUserId == userId && e.fromUserId == fromUserId) || (e.fromUserId == userId && e.toUserId == fromUserId));
  ...
  Future<GlobalResponse> actionSendMessage(
      BuildContext ctx, String chatId, String userId, String message, String file, String createdAt, String timeZone) async {
    try {
      SharedPreferences pref = await SharedPreferences.getInstance();
      isLoadingTrue(ctx);
      Map data = {};
  ...
  void addUserDbAll(List<User> users) async {
    for (var e in users) {
      ...
      SqlUser data =
          SqlUser(email: e.email, fullname: e.userDetail!.fullname, isOnline: e.isOnline!, status: e.userDetail!.status!, userId: e.userId);
